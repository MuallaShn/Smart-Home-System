...
# # Speak işlemleri için görev kuyruğu
# speak_queue = queue.Queue()

# def speak_worker():
#     """Speak kuyruğunu işleyen worker thread."""
#     while True:
#         text = speak_queue.get()  # Kuyruktan bir metin alır
#         if text is None:  # Kuyruğa 'None' gönderilirse thread sonlanır
#             break
#         try:
#             engine.say(text)
#             engine.runAndWait()
...
#         finally:
#             speak_queue.task_done()  # İşlemi tamamladığını bildirir

# # Speak işlemlerini başlatan thread
# speak_thread = threading.Thread(target=speak_worker, daemon=True)
# speak_thread.start()

# def speak(text):
#     """Metni konuşma kuyruğuna ekler."""
